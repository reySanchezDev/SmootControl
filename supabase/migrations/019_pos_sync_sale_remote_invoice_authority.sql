-- Makes Supabase the authority for POS invoice numbers.
-- Local POS invoice numbers are treated as provisional for new sales.
...
BEGIN
  ...
  SELECT id, invoice_number
    INTO existing_sale_id, existing_invoice_text
    FROM public.sales
   WHERE restaurant_id = p_restaurant_id
     AND (
       id = v_sale_id
       OR (
         local_sale_id IS NOT NULL
         AND local_id = local_sale_id
       )
     )
   ORDER BY CASE WHEN id = v_sale_id THEN 0 ELSE 1 END
   LIMIT 1;

  IF existing_sale_id IS NOT NULL THEN
    v_sale_id := existing_sale_id;
    invoice_text := existing_invoice_text;
    renumbered_invoice := invoice_text IS DISTINCT FROM original_invoice_text;
  ELSE
    ...
    SELECT COALESCE(NULLIF(prefix, ''), 'F'), next_number
  ...
  IF void_payload IS NOT NULL AND void_payload <> '{}'::jsonb THEN
    ...
    ON CONFLICT ON CONSTRAINT sale_voids_sale_id_key DO UPDATE
...
COMMENT ON FUNCTION public.pos_sync_sale(
  ...
  'Synchronizes one POS sale idempotently and assigns invoice numbers remotely.';
