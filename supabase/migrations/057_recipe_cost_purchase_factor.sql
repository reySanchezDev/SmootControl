...
AS $$
  ...
  SELECT COALESCE(
    SUM(
      tree.required_base_qty *
      (component.cost / NULLIF(component.purchase_to_inventory_factor, 0))
    ),
    0
  )
  FROM recipe_tree tree
...
COMMENT ON FUNCTION public.app_recipe_unit_cost(uuid, uuid) IS
  'Returns recipe unit cost using raw material purchase cost divided by purchase-to-inventory factor.';
