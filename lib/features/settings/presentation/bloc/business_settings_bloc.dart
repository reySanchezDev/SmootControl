      ...
      case AppSuccess():
        await _auditLogRepository.saveEntry(
          AuditLogEntry(
            ...
            details: {
              'businessName': event.settings.businessName,
              'invoicePrefix': event.settings.invoicePrefix,
            },
            occurredAt: DateTime.now(),
          ),
        );
        final loadResult = await _repository.getSettings();
        emit(
          loadResult.when(
            success: (settings) => BusinessSettingsLoaded(
              settings,
              saved: true,
            ),
            failure: BusinessSettingsFailure.new,
